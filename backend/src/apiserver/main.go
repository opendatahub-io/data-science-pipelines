...
func initCerts() (*tls.Config, error) {
	if *tlsCertPath == "" && *tlsCertKeyPath == "" {
		...
		return nil, nil
	} else if *tlsCertPath == "" || *tlsCertKeyPath == "" {
		// Both are required
		return nil, fmt.Errorf("Missing either tlsCertPath or tlsCertKeyPath, when specifying cert paths, both are required.")
	}
...
func startRpcServer(resourceManager *resource.ResourceManager, config *tls.Config) {
	var s *grpc.Server
	if config != nil {
...
func registerHttpHandlerFromEndpoint(handler RegisterHttpHandlerFromEndpoint, serviceName string, ctx context.Context, mux *runtime.ServeMux, tlsConfig *tls.Config) {
	...
	if tlsConfig != nil {
		// local client connections to grpc should not require tls
