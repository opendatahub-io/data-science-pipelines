	...
	dag := &wfapi.Template{
		...
		Inputs: wfapi.Inputs{
			Parameters: append([]wfapi.Parameter{
				{Name: paramParentDagID}},
				c.getTaskRetryParameters(name, name)...),
		},
		DAG: &wfapi.DAGTemplate{},
	}
	retrySpec := c.getTaskRetryPolicySpec(name, name)
	if name != "root" && retrySpec != nil {
		dag.RetryStrategy = c.getTaskRetryStrategyFromInput(inputParameter(paramRetryMaxCount),
...
func (c *workflowCompiler) dagTask(name string, componentName string, inputs dagInputs) *wfapi.DAGTask {
	...
	if name != "root" {
		task.Arguments.Parameters = append(task.Arguments.Parameters,
			c.getTaskRetryParametersWithValues(name, componentName)...)
	}
