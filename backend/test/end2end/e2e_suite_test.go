...
var _ = BeforeSuite(func() {
	...
	var newArtifactClient func() (*apiserver.ArtifactClient, error)
	...
	if *config.KubeflowMode {
		...
		newArtifactClient = func() (*apiserver.ArtifactClient, error) {
			return apiserver.NewKubeflowInClusterArtifactClient(*config.Namespace, *config.DebugMode, tlsCfg)
		}
		...
		newArtifactClient = func() (*apiserver.ArtifactClient, error) {
			return apiserver.NewMultiUserArtifactClient(clientConfig, userToken, *config.DebugMode, tlsCfg)
		}
	} else {
		...
		newArtifactClient = func() (*apiserver.ArtifactClient, error) {
			return apiserver.NewArtifactClient(clientConfig, *config.DebugMode, tlsCfg)
		}
	}
	...
	artifactClient, err = newArtifactClient()
	Expect(err).To(BeNil(), "Failed to get Artifact client")
})
