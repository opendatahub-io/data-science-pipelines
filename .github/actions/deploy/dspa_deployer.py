...
Handles generating the DSPA YAML manifest and deploying it via
the DSPO operator, including the two-stage wait pattern.
"""
    ...
    def generate_dspa_yaml(self) -> Dict[str, Any]:
        ...
        if self.args.storage_backend == 'minio':
            ...
            dspa_config['spec']['objectStorage'] = {
                'externalStorage': {
                    'host':
                        f'minio-service.{self.deployment_namespace}.svc.cluster.local',
                    'port':
                        '9000',
                    'bucket':
                        'mlpipeline',
