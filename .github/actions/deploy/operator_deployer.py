...
Handles cloning the operator repository, installing CRDs,
deploying the operator, and configuring it for external Argo.
"""
...
class OperatorDeployer:
    ...
    def _patch_params_for_kind(self):
        """Replace Red Hat registry images in params.env with publicly accessible alternatives.

        The operator's default params.env references registry.redhat.io images
        that require authentication. On Kind clusters there are no RH registry
        credentials, so these images must be swapped for public equivalents.
        """
