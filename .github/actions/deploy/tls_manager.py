...
"""
    ...
    def _copy_secret_between_namespaces(self, secret_name: str, source_ns: str,
        ...
        # Get the secret from source namespace and apply to target namespace
        copy_command = f"""
...
"""
            ...
            # Patch MariaDB deployment to mount TLS certificates and configuration
            patch_json = f'''[
              {{
                "op": "add",
            ...
            print(f'🔧 Patching MariaDB deployment to enable TLS...')
            self.deployer.deployment_manager.run_command([
                'kubectl', 'patch', 'deployment', 'mariadb', '-n',
                self.deployer.external_db_namespace, '--type=json', '-p',
                patch_json
            ])
...
"""
    ...
    def cleanup_certificates(self):
        ...
        try:
            ...
            # Delete ConfigMaps
            self.deployer.deployment_manager.run_command([
                'kubectl', 'delete', 'configmap', 'openshift-service-ca.crt',
                '-n', self.deployment_namespace
            ],
                                                         check=False)
